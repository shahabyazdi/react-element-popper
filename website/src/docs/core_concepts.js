...
export default function (translate, language) {
    ...
    jsx: (
      <>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div
              style={{
                marginBottom: "5px",
                [language === "en" ? "marginLeft" : "marginRight"]: `-${
                  language === "en" ? "20" : "12"
                }px`,
              }}
            >
        ...
        {translate("popper_larger_parent").map((string, index) => (
          <div
            key={index}
            dangerouslySetInnerHTML={{ __html: `<p>${string}</p>` }}
          />
        ))}
      </>
    ),
    ...
    description: (
      <ol>
        {translate("important_notes").map((note, index) => (
          <div
            key={index}
            dangerouslySetInnerHTML={{ __html: `<li>${note}</li>` }}
          />
        ))}
      </ol>
    ),
  ...
  return [howItWork, popperDescription, importantNotes];
}
